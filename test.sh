...
# Test Case 2
env='PLUGIN_REPO=test_repo PLUGIN_AUTO_TAG=true PLUGIN_CONTEXT=/tmp PLUGIN_DOCKERFILE=docker/Dockerfile PLUGIN_JSON_KEY=test_json_key'
output='/kaniko/executor -v info --context=/tmp --dockerfile=docker/Dockerfile --destination=index.docker.io/test_repo:latest'
__run "$env" "$output"
__check_file ".tags" "latest"
